...
outputs - tensor of shape (L, D*H_out) for unbatched input, (L, N, D*H_out) for batch input
        ...
        - means it contains all h_t from h_1 to h_t of last LSTM layer.
...
class Seq2Seq(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, target_size: int, output_size: int, batch_size: int,
        ...
        : param input_size  : number of x features.
    ...
    def forward(self, input_tensor: Tensor, target_tensor: Tensor, short_term: Tensor = None,
                long_term: Tensor = None, att: bool = False) -> tuple[Tensor, Tensor, Tensor]:
        """

        : param input_tensor  : dataset input X.
        : param target_tensor : dataset input y.
        : param short_term    : hidden states of encoder.
        : param long_term     : cell states of encoder.
        : param att           : choose whether to use self-attention.
        """
        encoder_outputs, (encoder_hn, encoder_cn) = self.encoder(input_tensor, short_term, long_term,
                                                                 device=self.device)
        ...
        if att:
            ...
            # the encoder_outputs is source of k, v
            encoder_outputs = encoder_outputs.transpose(0, 1)

            # reshape to (batch, seq, feature)
            # the target_tensor is the source of q
            target_tensor = target_tensor.transpose(0, 1)

            input_dim = encoder_outputs.shape[-1]
            input_qdim = target_tensor.shape[-1]

            att = self_attention.SelfAttention(input_dim=input_dim, q_dim=64, v_dim=1, input_qdim=input_qdim)

            att_out = att(target_tensor, encoder_outputs, encoder_outputs)

            target_tensor = att_out.transpose(0, 1)
